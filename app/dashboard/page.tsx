...
export default function DashboardAdmin() {
  ...
  const fetchDashboardData = async () => {
    try {
      const { data: resumen } = await supabase.from('resumen_ejecutivo_admin').select('*').single()
      const { data: atraso } = await supabase.from('vista_clientes_atraso').select('*').limit(10)
      setMetrics({
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-red-600 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <button onClick={() => router.push('/creditos/nuevo')} className="bg-red-700 px-4 py-2 rounded-lg hover:bg-red-800">
            + Nuevo Crédito
          </button>
        </div>
      </header>
                ...
                {clientesAtraso.map((cliente: any) => (
                  <div key={cliente.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{cliente.dias_atraso} días - S/ {cliente.monto_cuota}</p>
                    </div>
                    <a href={generarWhatsAppLink(cliente.telefono, `Hola ${cliente.nombre}, te escribe REDIMACOL.`)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600">
                      <Phone size={16} />
                      WhatsApp
                    </a>
                  </div>
